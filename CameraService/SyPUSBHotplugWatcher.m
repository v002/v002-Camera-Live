...
{
    SyPUSBHotplugWatcher *watcher = (__bridge SyPUSBHotplugWatcher *)info;
    [watcher handleUSBEvents];
}

static void usb_pollfd_added(int fd, short events, void *user_data)
{
    SyPUSBHotplugWatcher *watcher = (__bridge SyPUSBHotplugWatcher *)user_data;
...
- (BOOL)setupUSB
{
    ...
    {
        libusb_set_pollfd_notifiers(_USBContext, usb_pollfd_added, usb_pollfd_removed, (__bridge void *)(self));
        const struct libusb_pollfd **list = libusb_get_pollfds(_USBContext);
        int count = 0;
        while (list && list[count])
        {
            usb_pollfd_added(list[count]->fd, list[count]->events, (__bridge void *)(self));
            count++;
        }
    ...
    if (result == LIBUSB_SUCCESS)
    {
        dispatch_async(_USBQueue, ^{
            int result = libusb_hotplug_register_callback(self->_USBContext,
    ...
    if (result == LIBUSB_SUCCESS)
    {
        ...
        if (result == 1)
        {
            ...
            if (!_timer)
            {
                ...
                dispatch_set_context(_timer, (__bridge void * _Nullable)(self));
...
- (void)handleUSBAddedFD:(int)fd forEvents:(short)events
{
    ...
    dispatch_set_context(source, (__bridge void * _Nullable)(self));
...
- (void)postError:(int)code
{
    dispatch_async(_userQueue, ^{
        ...
        self->_handler(SypUSBHotplugError, error);
    });
}
